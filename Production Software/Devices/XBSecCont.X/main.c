/* 
 * XBee Sector Controller
 * Author:  Michael King
 * 
 * Created on July 7, 2024
 * 
 * This is meant to be a generic Sector Light controller on the wireless network
...
#define mainWXC_TASK_PRIORITY (tskIDLE_PRIORITY + 1)
...
void prvXSCTask( void * parameters );
    ...
    for(;;)
    {
        ...
        while((xQueueReceive(xDeviceIN_Queue, buffer, 0) == pdTRUE) && (breakloop == 0))
        {
            ...
            switch(buffer[0])
            {
                ...
                case 'S': //Status task, should only be the low battery message
                {
                    switch(buffer[1])
                    {
                        case 'B': //battery warning
                            lowbatt = 1;
                            updateIND = 1;
                            break;
                    }
