...
\begin{document}
    ...
    \section{Introduction}
     ...
     \newpage
     \section{operations}
     	see figure~\ref{fig:simpleflowgateway} for a simplified flowchart of the operation of the Master
     	...
     	\subsection{coordinator}
     		The coordinator's primary task is to perform coordination between the wired and wireless
     		 ...
     		 As such, it needs to be capable of keeping a large amount of device names in memory for 
     ...
     \begin{figure}[htp]
     	\begin{tikzpicture}[auto, node distance=3cm,>=latex']
     		...
     		\node[startstop, right of=errcheck, xshift=1cm](display){display on user interface};
     		\node[decision, below of=errcheck, yshift=-1cm](collcheck){collision?};
     		\node[startstop, right of=collcheck, xshift=1cm](reassign){}
     		
     	\end{tikzpicture}
     	\caption{simple gateway flow chart}\label{fig:simpleflowgateway}
     \end{figure}
\end{document}